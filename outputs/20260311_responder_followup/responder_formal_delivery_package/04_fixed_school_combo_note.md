# 固定校区组合版 split：结果说明 + sensitivity analysis

> 本页用于正式结果说明，单独回答“若固定若干校区训练、另外若干校区测试，结论会怎样变化”，并把已选主方案 B 与补跑的 sensitivity 方案 A 放在同一页中，便于接收方快速理解 train/test 组合设计与边界判断。
...
- **方案 B** 在 `clinical_baseline_main` 上更好，尤其 out-phase 仍能保持在 **0.5325** 左右，因此它更适合作为当前正式说明的主固定 split。
...
> 可直接用于正式说明的一句话：固定校区组合版 split 我们现在已经有主方案 B，也补跑了方案 A 做 sensitivity；两套切法结论方向一致——当前信号可以说存在，但还不足以写成强跨校区泛化或 external validation。
