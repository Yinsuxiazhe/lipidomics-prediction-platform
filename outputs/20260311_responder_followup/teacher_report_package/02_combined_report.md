# Responder 统一汇报包（基线主结果 + school split follow-up）

> 本页按“**先正式主结果，再补学校 grouped split 与 out-phase internal temporal validation，最后交代 protocol 写法与当前边界**”的顺序整理，适合直接给老师 / 合作者浏览。

**可直接发送：**

> 老师您好，这个包我按“先正式主结果、后补真实学校 grouped split、再看 out-phase internal temporal validation、最后明确 protocol 写法和当前边界”的顺序整理好了。核心不是把分数再做高，而是把目前最关键的口径讲清楚：strict nested CV outer-test AUC 仍约 0.50–0.54；学校 split 和 out-phase 都已补跑，但仍是内部验证，不是 external validation；AUC≈0.8 只对应 mean_train_auc。
...
## 学校 split 与 protocol 写法对应页

- 学校 / 社区 split 的新分析位置： [followup_plan_alignment.md](followup_plan_alignment.md)
...
- follow-up 图文页： [01_followup_report.html](01_followup_report.html)
